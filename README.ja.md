...
配布されるスキルは 108 件、3 つのドメインに分かれています。各名前の先頭 2 文字がドメインを表すので、フラットに並んだスキル一覧を見た人が、どれがこのパッケージ由来でどのドメインに属するかを一目で判別できます。
...
既定では全ドメインが入ります。Claude Code はインストール済みの全スキルの名前と説明を常時コンテキストに載せるため、バックエンドだけのリポジトリでもフロントエンドの 45 件分を毎ターン負担することになります。ドメイン単位で絞れます。
