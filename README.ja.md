...
配布されるスキルは 114 件、4 つのドメインに分かれています。各名前の先頭 2 文字がドメインを表すので、フラットに並んだスキル一覧を見た人が、どれがこのパッケージ由来でどのドメインに属するかを一目で判別できます。

| プレフィックス | ドメイン | スキル数 | 内容 |
| :-- | :-- | --: | :-- |
| `hc-` | `core` | 35 | スタックを問わず、あらゆるプロジェクトに適用される規約と手順 |
| `hb-` | `backend` | 31 | renchan ベースの Node バックエンド |
| `hf-` | `frontend` | 46 | Furo/Nuxt アプリ |
| `hs-` | `support` | 2 | 実装規約ではなく、開発の作業そのものを補助する付随スキル |
...
既定では全ドメインが入ります。スタックが片方だけのリポジトリには多すぎる量です。後述の「ドメインを絞る」で、同じ package.json から絞れます。
...
パッケージを依存に入れないまま実行する場合 — 一度だけの配置や、フックを置けないリポジトリ — は、名前を省略せずに指定してください。`npx --package=@openreachtech/hora-skills hora-skills install` が取得するのは、他人が公開できないスコープ付きの名前です。無スコープの bin 名には、その保証がありませんでした。

## 使い方

スキルは自分のリポジトリの `.claude/skills/` に配置されます。Claude Code はそこからスキルを認識し、それぞれが自身の名前で呼び出せるようになります(`/hc-naming`・`/hb-query-resolver`・`/hf-cp-table` など)。インストールされたスキルは、そのリポジトリ自身のスキルと 1 つのフラットな一覧に並びます。`hc-`/`hb-`/`hf-`/`hs-` のプレフィックスはそのためにあります。
...
リポジトリが自分で作ったスキルは、その名前がこのパッケージの配布名と一致しない限り削除されません。`hc-`/`hb-`/`hf-`/`hs-` のプレフィックスを持っているだけで対象になることはなく(`hc-own-skill` は残ります)、配布スキルと完全に同じ名前を付けた場合に限り、その名前はこのパッケージのものとして扱われます。
