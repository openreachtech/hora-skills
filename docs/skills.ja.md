...
このリポジトリに収録されている全 115 件のスキルの一覧と、それぞれの概要(1〜2 行)です。
...
36 件。スタックを問わず、あらゆるプロジェクトに適用される規約と手順です。
...
| `hc-test-cache` | 検証ユニットの入力が変わっていない間、記録済みのテスト成功を再利用します。ユニットの宣言、MISS/REUSED 行・抜き打ち監査・終了コードの読み方、記録が実行の代わりにならない場面。 |
...
2 件。実装規約ではなく、開発の作業そのものを補助する付随スキルです。

| スキル (= コマンド) | 概要 |
| :-- | :-- |
| `hs-explain` | AI が生成した説明を、専門知識のない読者(中学生を想定)が一読で理解できる形に組み直します。結論 1 文、必須の図、最大 3 つのポイント、用語ミニ辞書。 |
| `hs-user-manual` | 起動した E2E 環境をテストユーザーとして実際に操作し、エンドユーザー向け利用マニュアルを生成します。1 機能 1 HTML ページ+スクリーンショット、必須の目次、`docs/manuals/` 配下でプロダクトのバージョンに紐づけて管理。 |
