...
| `hb-build-e2e-test-environment` | Build, run and debug the hand-operated local E2E stack under `e2e/docker/` — its containers behind the same reverse-proxy edge production serves through, its seed set, and the `up`/`start`/`seed`/`clean`/`down` scripts. |
