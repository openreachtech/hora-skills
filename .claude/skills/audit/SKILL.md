...
Duplicate names are not checked, because they cannot occur. Two skills in one domain would need two folders of one name in one directory, and two skills in different domains have different prefixes — so once each `name:` is confirmed equal to its folder name, uniqueness follows from the filesystem rather than from a comparison.
...
It exits `0` when every skill sits one level under its domain and declares a `name:` equal to its folder name, and `1` on any of the failures above, so it can gate a commit or a CI job.
...
For a skill in the wrong place — nested too deep, or under a domain its prefix does not match — move the folder to the domain its prefix names, or change both the prefix and `name:` to the domain it actually belongs in. Renaming is the deliberate part: an installed skill's name is what consuming repositories invoke, so a rename is a breaking change for them.
