...
- Unlike the clients generator, this script has **no `--force` flag** — it always rewrites the single output file wholesale. Because the file is fully generated, keep the per-operation `RequestVariables`/`ResponseContent` typedefs in their Payload/Capsule files (never in this `.d.ts`) so they survive regeneration.
