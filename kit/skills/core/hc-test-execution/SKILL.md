...
description: >
  ...
  deleted, loosened, or padded with waits to make the suite pass. Use whenever tests are run,
  when a test still fails after an implementation was believed finished, when the user asks to run or
  fix failing tests, and before claiming an implementation is complete. Writing new tests belongs to
  the test-writing convention; reusing a recorded pass over unchanged inputs, to the test-cache
  convention.
---
...
and what a reused pass must be reported with, belong to `hc-test-cache`.
