...
| `tally` | A value that **serves double duty** as `input` and `expected`. When the same value is both passed in and used as the expected value, combine it into a single `tally` instead of splitting it into `input` and `expected` (e.g. pass it to `create(tally)` and also verify it with `toHaveBeenCalledWith(tally)`). The name means "tally" in the sense of a matching token, expressing the intent that the passed value and the expected value are **checked to be identical**. This name was chosen as a short word that does not collide with common domain vocabulary. Use it **only when the passed value and the expected value are the same**; if the implementation transforms the argument such that the two diverge, do not use `tally` — split it into `input` / `expected`. |
